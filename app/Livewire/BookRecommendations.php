...
class BookRecommendations extends Component
{
    ...
    public function loadRecommendations()
    {
        ...
        if ($auth) {
            ...
            $this->bookRecommendations = $books
                ->sortByDesc('score') // ưu tiên sách có điểm cao
                ->take(8)
                ->values();
        } else {
            $this->bookRecommendations = Book::inRandomOrder()->take(8)->get();
        }

        // Add discount information
        $this->bookRecommendations = $this->bookRecommendations->map(function($book) {
            $discount = $book->discounts()
                ->where('start_date', '<=', now())
                ->where('end_date', '>=', now())
                ->first();
            
            $book->discount_percent = $discount?->percent ?? 0;
            $book->discount_price = $book->discount_percent > 0 
