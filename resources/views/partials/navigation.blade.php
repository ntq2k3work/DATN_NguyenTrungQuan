      ...
      <li class="relative group py-4">
        ...
        <!-- Dropdown -->
            ...
            @if ($category->children->isEmpty())
              <li class="hover:bg-gray-100"><a href="{{ route('categories.show', $category->slug) }}" class="block w-full px-4 py-2">{{ $category->name }}</a></li>
            @else
              <li class="relative group px-0 py-0 business-book">
                  <a href="{{ route('categories.show', $category->slug) }}" class="flex items-center justify-between w-full px-4 py-2 hover:text-red-500">
                      {{ $category->name }}
                  ...
                  <ul class="business-book-menu absolute left-full top-0 w-48 bg-white shadow-lg rounded-lg hidden  z-50">
                      @foreach ($category->children as $children )
                        <li class=" hover:bg-gray-100"><a class="block w-full px-4 py-2" href="{{ route('categories.show', $children->slug) }}">{{ $children->name }}</a></li>
                      @endforeach
                  </ul>
              </li>
            @endif
