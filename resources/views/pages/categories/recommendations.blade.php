            ...
            <div class="group bg-amber-50 rounded-lg shadow hover:shadow-lg transition-shadow duration-300">
                <div class="p-3 sm:p-4">
                    <div class="relative mb-3 sm:mb-4">
                        ...
                        <button
                            class="absolute top-2 right-2 bg-white/80 hover:bg-white p-1.5 sm:p-2 rounded-full opacity-0 group-hover:opacity-100 transition-opacity"
                        >
                    ...
                    <div class="space-y-2 sm:space-y-3">
                        <div>
                            ...
                            <p class="text-xs sm:text-sm text-gray-500">{{ $book->author->name }}</p>
                        </div>
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-xs sm:text-sm text-gray-500">{{ number_format($book->rating ?? 4.8, 1) }} ({{ $book->reviews_count ?? rand(100, 2000) }})</span>
                        </div>

                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-2">
                                @if (isset($book->discount_percent) && $book->discount_percent > 0)
                                    <span class="text-base sm:text-lg font-bold text-primary">{{ number_format($book->discount_price ?? $book->final_price, 0, ',', '.') }}đ</span>
                                    <span class="text-xs sm:text-sm text-gray-500 line-through">{{ number_format($book->price, 0, ',', '.') }}đ</span>
                                @else
                                    <span class="text-base sm:text-lg font-bold text-primary">{{ number_format($book->price, 0, ',', '.') }}đ</span>
                                @endif
                            </div>
                        </div>

                        <button
                            class="w-full bg-amber-600 text-white text-xs sm:text-sm py-2 px-3 sm:px-4 rounded hover:bg-primary/90 transition"
                        >
                            Thêm vào giỏ
                        </button>
                    </div>
